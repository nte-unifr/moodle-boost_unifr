 ...
 * @return string
 */
function theme_boost_unifr_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_boost_unifr', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_boost_unifr and not theme_boost (see the line above).
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/boost_unifr/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/boost_unifr/scss/post.scss');
 ...
 * @return array
 */
function theme_boost_unifr_get_pre_scss($theme) {
    global $CFG;
    // MODIFICATION START.
    ...
    $scss .= theme_boost_unifr_get_loginbackgroundimage_scss();
    // MODIFICATION END.
 ...
 * @return bool
 */
function theme_boost_unifr_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $theme = theme_config::load('boost_unifr');
...
/**
 * If setting is updated, use this callback to clear the theme_boost_unifr' own application cache.
 */
function theme_boost_unifr_reset_app_cache() {
...
/**
 * If setting is updated, use this callback to reset the theme_boost_unifr_infobanner_dismissed user preferences.
 */
function theme_boost_unifr_infobanner_reset_visibility() {
    global $DB;

    if (get_config('theme_boost_unifr', 'perpibresetvisibility') == 1) {
        ...
        $params = ['name' => 'theme_boost_unifr_infobanner_dismissed', 'value' => '1'];
        ...
        foreach ($users as $user) {
            try {
                unset_user_preference('theme_boost_unifr_infobanner_dismissed', $user->userid);
        ...
        if (!$somethingwentwrong) {
            \core\notification::success(get_string('resetperpetualinfobannersuccess', 'theme_boost_unifr'));
        } else {
            \core\notification::error(get_string('resetperpetualinfobannervisibilityerror',
                    'theme_boost_unifr', $codingexception));
        }
