 ...
 */
function theme_boost_unifr_get_loginbackgroundimage_files() {
    ...
    if ($files == null) {
        ...
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_unifr', 'loginbackgroundimage',
            false, 'itemid', false);
    }
 ...
 * @throws dml_exception
 */
function theme_boost_unifr_get_random_loginbackgroundimage_number() {
    ...
    if ($number == null) {
        ...
        $files = theme_boost_unifr_get_loginbackgroundimage_files();
 ...
 * @return string
 */
function theme_boost_unifr_get_random_loginbackgroundimage_class() {
    // Get the static random number.
    $number = theme_boost_unifr_get_random_loginbackgroundimage_number();
 ...
 * @throws dml_exception
 */
function theme_boost_unifr_get_loginbackgroundimage_text() {
    ...
    if ($number != null) {
        ...
        $lines = explode("\n", get_config('theme_boost_unifr', 'loginbackgroundimagetext'));
 ...
 * @return string
 */
function theme_boost_unifr_get_loginbackgroundimage_scss() {
    $count = 0;
    $scss = "";
 ...
 * @return array
 */
function theme_boost_unifr_get_imageareacontent() {
    // Get cache.
    $themeboostcampuscache = cache::make('theme_boost_unifr', 'imagearea');
    ...
    if (!empty($cachecontent)) {
        ...
        $fs = get_file_storage();
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_unifr', 'imageareaitems', false, 'itemid', false);
        ...
        if (!empty($files)) {
            ...
            $lines = explode("\n", get_config('theme_boost_unifr', 'imageareaitemsattributes'));
            // Parse item settings.
            foreach ($lines as $line) {
                $line = trim($line);
 ...
 */
function theme_boost_unifr_process_flatnav(flat_navigation $flatnav) {
    global $USER;
    // If the setting defaulthomepageontop is enabled.
    if (get_config('theme_boost_unifr', 'defaulthomepageontop') == 'yes') {
        ...
        if (($coursehomenode = $flatnav->find('coursehome', global_navigation::TYPE_CUSTOM)) != false) {
            ...
            if (get_config('core', 'defaulthomepage') == HOMEPAGE_SITE) {
                ...
                $flatnavreturn = theme_boost_unifr_set_node_on_top($flatnav, 'home', $coursehomenode);
            } else if (get_config('core', 'defaulthomepage') == HOMEPAGE_MY) { // If the dashboard is set as the default homepage
                // by the admin.
                // Return the modified flat_navigation.
                $flatnavreturn = theme_boost_unifr_set_node_on_top($flatnav, 'myhome', $coursehomenode);
 ...
 * @return flat_navigation.
 */
function theme_boost_unifr_set_node_on_top(flat_navigation $flatnav, $nodename, $beforenode) {
 ...
 */
function theme_boost_unifr_get_incourse_settings() {
    global $COURSE, $PAGE;
    ...
    if (get_config('theme_boost_unifr', 'showsettingsincourse') == 'yes') {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE || $PAGE->context->contextlevel == CONTEXT_MODULE) {
            ...
            if (!empty($node)) {
                ...
                if (((get_config('theme_boost_unifr', 'incoursesettingsswitchtoroleposition') == 'yes') ||
                    (get_config('theme_boost_unifr', 'incoursesettingsswitchtoroleposition') == 'both'))
                    ...
                    if (is_array($roles) && (count($roles) > 0)) {
                        ...
                        $properties = array('text' => get_string('switchroleto', 'theme_boost_unifr'),
 ...
 * @return navigation_node.
 */
function theme_boost_unifr_get_incourse_activity_settings() {
    global $PAGE;
 ...
 * @return string.
 */
function theme_boost_unifr_get_course_guest_access_hint($courseid) {
    global $CFG;
    ...
    foreach ($instances as $instance) {
        ...
        if ($plugin->show_enrolme_link($instance)) {
            $html = html_writer::tag('div', get_string('showhintcourseguestaccesslink',
                'theme_boost_unifr', array('url' => $CFG->wwwroot . '/enrol/index.php?id=' . $courseid)));
            break;
        }
    }
 ...
 * @return boolean
 */
function theme_boost_unifr_show_banner_on_selected_page($infobannerpagestoshow, $infobannercontent, $thispagelayout,
 ...
 * @return boolean
 */
function theme_boost_unifr_show_timed_banner_on_selected_page($now, $timedibshowonpages, $timedibcontent, $timedibstartsetting,
    ...
    if (($now >= $timedibstart && $now <= $timedibend ||
        ...
        $timedinfobannershowonselectedpage = theme_boost_unifr_show_banner_on_selected_page($timedibshowonpages,
