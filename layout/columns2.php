...
if (get_config('theme_boost_unifr', 'catchendkey') == true) {
    $catchshortcuts[] = 'end';
}
// If setting is enabled then add the parameter to the array.
if (get_config('theme_boost_unifr', 'catchcmdarrowdown') == true) {
    $catchshortcuts[] = 'cmdarrowdown';
}
// If setting is enabled then add the parameter to the array.
if (get_config('theme_boost_unifr', 'catchctrlarrowdown') == true) {
    $catchshortcuts[] = 'ctrlarrowdown';
}
// MODIFICATION END.

// MODIFICATION START: Setting 'darknavbar'.
if (get_config('theme_boost_unifr', 'darknavbar') == 'yes') {
    $darknavbar = true;
} else {
    $darknavbar = false;
}
...
if (get_config('theme_boost_unifr', 'showsettingsincourse') == 'yes') {
    ...
    $templatecontext['node'] = theme_boost_unifr_get_incourse_settings();
    // Add the returned value from theme_boost_unifr_get_incourse_activity_settings to the template context.
